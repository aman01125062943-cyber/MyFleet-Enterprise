...
async function main() {
    console.log('🚀 Starting Pairing Code Request v3...');
    ...
    try {
        ...
        if (initData.sessionId) {
            ...
            console.log('⚠️ Session already exists in DB.');
            sessionId = initData.existingSessionId;
        } else {
            console.error('❌ Could not determine Session ID from response:', initText);
            return;
        }

        console.log(`🆔 Session ID: ${sessionId}`);

        // 2.5 Force Disconnect first to ensure clean state
        console.log('🔌 forcing disconnect to ensure clean state...');
        await fetch(`${SERVICE_URL}/api/sessions/${sessionId}/disconnect`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
            }
        }).catch(err => console.error('Disconnect warning (ignorable):', err.message));

        // 2.6 Trigger Connection Start (via Reconnect)
